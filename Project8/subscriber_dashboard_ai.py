...
FEATURE_NAMES = ["pressure", "flow", "gate"]
...
def extract_features(readings):
    ...
    """
    features = [
        readings.get("pressure", readings.get("pressure_upstream", 0)),
        readings.get("flow", readings.get("flow_rate", 0)),
        readings.get("gate", readings.get("gate_a_position", 50)),
    ]
...
def score_with_ai(readings):
    ...
    print("DEBUG features:", features.tolist())
    print("DEBUG model type:", type(ai_model))
    print("DEBUG prediction raw:", ai_model.predict(features))
    print("DEBUG score raw:", repr(float(ai_model.decision_function(features)[0])))
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        if accepted:
            ...
            print("DEBUG readings:", sensor_data)
