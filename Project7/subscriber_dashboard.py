    ...
    try:
        ...
        if accepted:
            ...
            if dashboard:
             ...
             # --- CUSTOM ALERT: High pressure ---
            pressure = data.get("readings", {}).get("pressure_upstream", 0)
            if pressure > 60:
                print(f"  [ALERT] High pressure detected: {pressure} PSI")
                if dashboard:
                    dashboard.log_anomaly_message(
                        reason=f"Pressure increased to {pressure} PSI",
                        anomaly_type="High Pressure",
                        source=device,
                        topic=msg.topic,
                    )

            # --- CUSTOM ALERT: Low flow ---
            if flow < 47:
                print(f"  [ALERT] Low flow detected: {flow} LPM")

                if dashboard:
                    dashboard.log_anomaly_message(
                        reason=f"Flow dropped to {flow} LPM",
                        anomaly_type="Low Flow",
                        source=device,
                        topic=msg.topic,
                    )
        else:
