...
class DashboardServer:
    ...
    def log_rejected_message(self, reason, attack_type, source, topic=""):
        ...
        # Backward-compatible safety: treat "Anomaly:*" as a warning, not an attack.
        if isinstance(attack_type, str) and attack_type.lower().startswith("anomaly"):
            self.log_anomaly_message(
                reason=reason,
                anomaly_type=attack_type.replace("Anomaly:", "").strip() or "Anomaly",
                source=source,
                topic=topic,
            )
            return
    ...
    def log_anomaly_message(self, reason, anomaly_type, source, topic=""):
        """Called when readings are suspicious but NOT a security attack."""
        self.stats["total"] += 1
        ...
        message = {
            ...
            "data": {
                ...
                "timestamp": time.strftime("%H:%M:%S"),
                "description": f"Operational anomaly detected: {anomaly_type}",
            },
        }
        self._schedule_broadcast(message)
        logger.info(f"ANOMALY: {anomaly_type} from {source}")
